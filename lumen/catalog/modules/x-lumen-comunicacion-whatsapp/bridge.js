...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.lumen', 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(process.env.HOME || '~', '.lumen', 'whatsapp', 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(process.env.HOME || '~', '.lumen', 'whatsapp', 'document_cache');
const AUDIO_CACHE_DIR = path.join(process.env.HOME || '~', '.lumen', 'whatsapp', 'audio_cache');
...
if (PAIR_ONLY) {
    ...
    console.log('[INFO] WhatsApp pairing mode');
    console.log(`[INFO] Session: ${SESSION_DIR}`);
    console.log();
    startSocket();
} else {
    app.listen(PORT, '127.0.0.1', () => {
        console.log(`[INFO] Lumen WhatsApp bridge listening on port ${PORT} (mode: ${WHATSAPP_MODE})`);
        console.log(`[INFO] Session stored in: ${SESSION_DIR}`);
        if (ALLOWED_USERS.size > 0) {
            console.log(`[INFO] Allowed users: ${Array.from(ALLOWED_USERS).join(', ')}`);
        } else {
            console.log(`[WARN] No WHATSAPP_ALLOWED_USERS set — all messages will be processed`);
        }
        console.log();
        startSocket();
    });
