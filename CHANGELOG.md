...
### Fixed
- **Responsive layout on mobile (< 720px)**: Settings sidebar (240px fixed) now collapses to 64px icon-only on all 7 settings pages. Dashboard composer stacks vertically (input full-width + button below). Chat bubbles expand to 85% width. Marketplace grid becomes single-column. Topbar wraps on overflow.
